...
def flatten_products(raw: dict) -> List[dict]:
    ...
    for bank in raw.get("banks", []):
        ...
        for acct in bank.get("accounts", []):
            ...
            min_bal = acct.get("min_balance") or 0
            min_bal_str = f"${min_bal:,.0f}" if min_bal else "$0"
            fee = acct.get("monthly_fee") or 0
            ...
            product = {
                ...
                "base_rate_pct": f"{(acct.get('base_rate_pct') or 0):.2f}%",
                "base_rate_num": float(acct.get("base_rate_pct") or 0),
                ...
                "cdic_notes": acct.get("cdic_notes") or "",
                "debit_transactions": acct.get("debit_transactions") or "Not specified",
                "carried_forward": acct.get("carried_forward", False),
                "carried_forward_note": acct.get("carried_forward_note") or "",
                "source_date": source_info.get("source_date", ""),
                "is_stale": source_info.get("is_stale", False),
            }
...
def build_full_context() -> str:
    ...
    for p in data["products"]:
        ...
        # CDIC details with notes
        cdic_str = "Yes" if p["cdic_insured"] else "No"
        if p.get("cdic_partner"):
            cdic_str += f" (via {p['cdic_partner']})"
        if p.get("cdic_notes"):
            cdic_str += f" - {p['cdic_notes']}"

        # e-Transfer with restrictions
        etransfer_str = p.get("e_transfer_details") or ("Yes" if p["e_transfer"] else "No")

        # Joint account with conditions
        joint_str = "Yes" if p["joint_account"] else "No"
        if p.get("joint_account_details"):
            joint_str += f" ({p['joint_account_details']})"

        # Data freshness indicator
        freshness = ""
        if p.get("carried_forward"):
            freshness = f" | WARNING: CARRIED FORWARD - {p.get('carried_forward_note', 'not freshly confirmed')}"
        if p.get("is_stale"):
            freshness += " | WARNING: DATA MAY BE STALE"
        ...
        lines.append(
            ...
            f"  CDIC: {cdic_str}\n"
            f"  e-Transfer: {etransfer_str}\n"
            f"  Joint Account: {joint_str}\n"
            f"  Debit/Withdrawal Policy: {p.get('debit_transactions', 'Not specified')}\n"
            f"  Rate Tiers: {p['rate_tiers']}\n"
            f"  Key Features: {p['key_features']}\n"
            f"  Recent Changes: {p.get('new_features', 'None')}\n"
            f"  Updated: {p['last_updated']} | Source: {p['source']}{freshness}"
        )
...
3. When comparing banks, structure your analysis by: Rate Comparison, Fee Structure, Features (CDIC, e-Transfer, joint accounts), Withdrawal/Transaction Policy, Promo Analysis, and Strategic Assessment.
...
6. When asked "which is best", always clarify: best for what? (highest base rate, best promo, most features, best for large balances, lowest withdrawal friction, etc.)
7. Note important caveats like: notice periods (EQ Bank), tiered rates that require high balances (TD Growth, Wealthsimple), withdrawal penalties (Scotiabank MomentumPLUS forfeits interest), or restrictions on joint accounts (Neo: married/common-law only).
8. Always mention the data source date so analysts know freshness. If data is flagged as CARRIED FORWARD or STALE, explicitly warn the user.
...
11. ALWAYS mention withdrawal/debit transaction policies when comparing accounts. A 4% rate means nothing if you lose it on withdrawal.
12. When discussing CDIC coverage, mention the partner institution and aggregation risks (e.g., Neo via Peoples Bank  - deposits aggregate with direct Peoples Bank holdings).
13. When discussing joint accounts, ALWAYS mention the eligibility conditions (e.g., Neo: married/common-law couples only, not friends/business partners).
...
async def agent_compare(request: CompareRequest, req: Request):
    ...
    if not allowed:
        raise HTTPException(
            status_code=429,
            detail=f"You've used all {DAILY_LIMIT} free queries for today. Come back tomorrow!",
        )
