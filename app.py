...
Reads structured data parsed from your bank analysis document.
...
# ──────────────────────────────────────────────
# Each visitor gets 10 AI queries per day. Browsing the table is unlimited.
DAILY_LIMIT = int(os.environ.get("DAILY_QUERY_LIMIT", "10"))
_usage: dict[str, list[float]] = defaultdict(list)


def check_rate_limit(ip: str) -> tuple[bool, int]:
    """Check if an IP has queries remaining today. Returns (allowed, remaining)."""
    now = time.time()
    day_ago = now - 86400
    # Clean old entries
...
def load_data() -> dict:
    """Load the full bank data from JSON."""
...
def build_full_context() -> str:
    ...
    for p in data["products"]:
        promo = f" | PROMO: {p['promotional_rate']} for {p['promo_duration']}" if p["promotional_rate"] not in ("N/A", None) else ""
        lines.append(
            f"--- {p['bank_name']} - {p['account_name']} ---\n"
            f"  Bank Type: {p['bank_type']}\n"
            f"  Base Rate: {p['base_rate_pct']}{promo}\n"
            f"  Promo Details: {p.get('promo_details', 'N/A')}\n"
            f"  Min Balance: {p['minimum_balance']} | Fee: {p['monthly_fee']}\n"
            f"  CDIC: {'Yes' if p['cdic_insured'] else 'No'} | "
            f"e-Transfer: {p.get('e_transfer_details', 'Yes' if p['e_transfer'] else 'No')} | "
    ...
    disclaimers = "\n".join(f"  - {d}" for d in data.get("metadata", {}).get("disclaimers", []))

    return (
        f"DATABASE: {len(data['products'])} savings products across "
        f"{data['metadata'].get('total_banks', '?')} Canadian banks\n"
        f"Data as of: {data['metadata'].get('last_full_refresh', 'unknown')}\n\n"
        + "\n\n".join(lines)
        + f"\n\nDISCLAIMERS:\n{disclaimers}"
    )
...
You have access to a curated database of 18 savings products across 10 Canadian banks (Big 5, digital banks, and fintechs).
...
- Neo Financial 4.00% is NOT a promo — it's a stable base rate. This is the highest non-promotional rate in Canada.
- EQ Bank Notice Savings accounts require 10-day or 30-day withdrawal notice — higher rate but restricted liquidity.
- Wealthsimple rates are tier-dependent: Core (1.25%), Premium (1.75%), Generation (2.25%) — most customers get Core.
- CIBC eAdvantage has the highest promo rate (5.25%) but only for 4 months.
- Many Big 5 "High Interest Savings" accounts actually offer near-zero rates (0.01%).
- Tangerine promo is ending March 16, 2026 — flag this if relevant.
- Simplii promo runs through April 30, 2026.
...
async def list_all_products():
    """Return all 18 savings products with full details."""
    data = load_data()
    return {
...
async def top_rates(limit: int = 10, include_promos: bool = False):
    ...
    data = load_data()
    if include_promos:
        # Sort by highest number between base and promo
